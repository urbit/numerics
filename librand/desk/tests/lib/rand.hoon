  ::  /tests/lib/rand
::::
::    Milestones 1-6 (rand-spec.md): ++split-mix, ++philox, ++seed, +fork,
::    ++uni, ++pcg, ++dist (++rd + ++rs), ++sample.  The SplitMix64 and
::    Philox4x32-10 KAT vectors are checked against their published
::    reference values (Vigna's reference C for SplitMix64; the Random123
::    kat_vectors file -- all-zero, all-0xffffffff, and the pi-digits
::    vector -- for Philox), each cross-checked against an independent
::    Python re-implementation before being transcribed here.  ++uni's
::    float arms (+rs/+rd/+rh/+rq/+rs-oo/+rd-oo) are exact bit
::    constructions (+sun then a power-of-two multiply), so their expected
::    values are likewise checked against an independent Python IEEE-754
::    encoder, not the Hoon float printer.  ++pcg's KAT (seed=42, seq=54,
::    pcg-c's own demo convention) is checked against an independent
::    Python re-implementation of pcg-c's reference C (include/
::    pcg_variants.h) -- this also exercises the CORRECTED advance-then-
::    output order (rand-spec.md section 3.3; an earlier spec draft had
::    it backwards).  ++dist's value spot-checks (including the alias
::    table in ++sample) involve /lib/math transcendentals (log/sqrt), so
::    their expected values are read off this same ship's actual output
::    (sanity-checked by hand against the closed-form algorithm first)
::    rather than independently rederived in Python, which isn't
::    guaranteed to match this codebase's correctly-rounded kernels to the
::    last bit; ++dist's moment tests are regression checks against the
::    THEORETICAL targets (mean/variance) at a fixed seed, with tolerances
::    wide enough to absorb real sampling noise at n=50000 but tight
::    enough to catch an actual algorithm bug.  +from-atom, +fold-wide,
::    +mix, +fork, +bits, and +below have no external reference (they are
::    this library's own design, or Lemire's algorithm applied to this
...
::    sensitivity/unbiasedness properties instead.  Two real bugs were
::    caught by this on-ship testing during milestone 6 (alias-table's
::    worklist assignment reversed; +binomial comparing against the wrong
::    accumulator) -- see NEXT-STEPS.md.
::
/+  *test,
    rand,
    math
...
++  test-normal  ^-  tang
  %+  expect-eq
    ...
    !>(out:(normal:rd:dist:rand (from-atom:seed:rand %sm64 0)))
...
++  test-normal-determinism  ^-  tang
  %+  expect-eq
    !>((normal:rd:dist:rand (from-atom:seed:rand %phil 7)))
    !>((normal:rd:dist:rand (from-atom:seed:rand %phil 7)))
++  test-normal-mv  ^-  tang
  %+  expect-eq
    !>(`@rd`.~8.104012210055275)
    !>(out:(normal-mv:rd:dist:rand (from-atom:seed:rand %sm64 0) .~10 .~2))
++  test-normal-mv-bad-sigma  ^-  tang
  %-  expect-fail
  |.((normal-mv:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0 .~-1))
...
++  test-expon  ^-  tang
  %+  expect-eq
    ...
    !>(out:(expon:rd:dist:rand (from-atom:seed:rand %sm64 0) .~2))
++  test-expon-bad-rate  ^-  tang
  %-  expect-fail
  |.((expon:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0))
::  alpha=2 (>=1, direct Marsaglia-Tsang path).
++  test-gamma-ge1  ^-  tang
  %+  expect-eq
    !>(`@rd`.~0.7179344171650754)
    !>(out:(gamma:rd:dist:rand (from-atom:seed:rand %sm64 0) .~2))
::  alpha=0.5 (<1, boost path: gamma(1.5) * u^(1/0.5)).
++  test-gamma-lt1  ^-  tang
  %+  expect-eq
    !>(`@rd`.~0.05447897345403265)
    !>(out:(gamma:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0.5))
++  test-gamma-bad-shape  ^-  tang
  %-  expect-fail
  |.((gamma:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0))
++  test-beta  ^-  tang
  %+  expect-eq
    !>(`@rd`.~0.23622515961139348)
    !>(out:(beta:rd:dist:rand (from-atom:seed:rand %sm64 0) .~2 .~3))
++  test-chi2  ^-  tang
  %+  expect-eq
    !>(`@rd`.~0.8261342997997718)
    !>(out:(chi2:rd:dist:rand (from-atom:seed:rand %sm64 0) 3))
++  test-chi2-bad-df  ^-  tang
  %-  expect-fail
  |.((chi2:rd:dist:rand (from-atom:seed:rand %sm64 0) 0))
++  test-student-t  ^-  tang
  %+  expect-eq
    !>(`@rd`.~-0.7137613421937223)
    !>(out:(student-t:rd:dist:rand (from-atom:seed:rand %sm64 0) 5))
++  test-student-t-bad-df  ^-  tang
  %-  expect-fail
  |.((student-t:rd:dist:rand (from-atom:seed:rand %sm64 0) 0))
::  u ~ 0.0205 < 0.5 -> %.y.
++  test-bernoulli  ^-  tang
  %+  expect-eq
    !>(`?`%.y)
    !>(out:(bernoulli:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0.5))
++  test-bernoulli-bad-prob  ^-  tang
  %-  expect-fail
  |.((bernoulli:rd:dist:rand (from-atom:seed:rand %sm64 0) .~1.5))
++  test-geometric  ^-  tang
  %+  expect-eq
    !>(`@ud`11)
    !>(out:(geometric:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0.3))
::  p=1 edge case: returns 1 without consuming a draw (rng unchanged).
++  test-geometric-p1  ^-  tang
  =/  r  (from-atom:seed:rand %sm64 0)
  =^  out  r  (geometric:rd:dist:rand r .~1)
  ;:  weld
    %+  expect-eq  !>(`@ud`1)  !>(out)
...
++  test-geometric-bad-prob  ^-  tang
  %-  expect-fail
  |.((geometric:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0))
...
++  test-moments-normal  ^-  tang
  =/  mv  (moments (from-atom:seed:rand %phil 0) normal:rd:dist:rand 50.000)
  ;:  weld
    %+  expect-eq  !>(%.y)  !>((~(is-close rd:math [%n .~0 .~0.02]) -.mv .~0))
    %+  expect-eq  !>(%.y)  !>((~(is-close rd:math [%n .~0 .~0.05]) +.mv .~1))
  ==
++  test-moments-expon  ^-  tang
  =/  mv  (moments (from-atom:seed:rand %phil 0) |=(r=rng:rand (expon:rd:dist:rand r .~1)) 50.000)
  ;:  weld
    %+  expect-eq  !>(%.y)  !>((~(is-close rd:math [%n .~0 .~0.05]) -.mv .~1))
    %+  expect-eq  !>(%.y)  !>((~(is-close rd:math [%n .~0 .~0.05]) +.mv .~1))
  ==
++  test-moments-gamma  ^-  tang
  =/  mv  (moments (from-atom:seed:rand %phil 0) |=(r=rng:rand (gamma:rd:dist:rand r .~2)) 50.000)
  ;:  weld
...
++  moments
  ...
  ==
::  +shuffle: value regression, and the permutation property (same
::  multiset) over a fresh draw at a different seed.
++  test-shuffle  ^-  tang
  %+  expect-eq
    !>(`(list @ud)`~[3 4 1 2 5])
    !>(-:(shuffle:sample:rand (from-atom:seed:rand %sm64 0) `(list @ud)`~[1 2 3 4 5]))
++  test-shuffle-same-multiset  ^-  tang
  =/  in  `(list @ud)`~[10 20 30 40 50 60]
  =/  out  -:(shuffle:sample:rand (from-atom:seed:rand %phil 3) in)
  %+  expect-eq  !>((sort in lth))  !>((sort out lth))
::  +permutation: value regression (delegates to +shuffle over gulf 0 4).
++  test-permutation  ^-  tang
  %+  expect-eq
    !>(`(list @ud)`~[2 3 0 1 4])
    !>(-:(permutation:sample:rand (from-atom:seed:rand %sm64 0) 5))
::  +choice: value regression, and crash on empty.
++  test-choice  ^-  tang
  %+  expect-eq
    !>(30)
    !>(-:(choice:sample:rand (from-atom:seed:rand %sm64 0) `(list @ud)`~[10 20 30]))
++  test-choice-empty  ^-  tang
  %-  expect-fail
  |.((choice:sample:rand (from-atom:seed:rand %sm64 0) `(list @ud)`~))
::  +choices: value regression (with replacement -- repeats are fine).
++  test-choices  ^-  tang
  %+  expect-eq
    !>(`(list @ud)`~[30 20 10 30])
    !>(-:(choices:sample:rand (from-atom:seed:rand %sm64 0) 4 `(list @ud)`~[10 20 30]))
::  +sample-n: value regression, and crash if k exceeds the list length.
++  test-sample-n  ^-  tang
  %+  expect-eq
    !>(`(list @ud)`~[3 2 5])
    !>(-:(sample-n:sample:rand (from-atom:seed:rand %sm64 0) 3 `(list @ud)`~[1 2 3 4 5]))
++  test-sample-n-bad-count  ^-  tang
  %-  expect-fail
  |.((sample-n:sample:rand (from-atom:seed:rand %sm64 0) 6 `(list @ud)`~[1 2 3 4 5]))
::  +reservoir: value regression, and crash if n exceeds the list length.
++  test-reservoir  ^-  tang
  %+  expect-eq
    !>(`(list @ud)`~[8 2 5])
    !>(-:(reservoir:sample:rand (from-atom:seed:rand %sm64 0) 3 `(list @ud)`~[1 2 3 4 5 6 7 8]))
++  test-reservoir-bad-count  ^-  tang
  %-  expect-fail
  |.((reservoir:sample:rand (from-atom:seed:rand %sm64 0) 9 `(list @ud)`~[1 2 3 4 5 6 7 8]))
...
++  test-alias-build  ^-  tang
  %+  expect-eq
    !>(`alias-table:sample:rand`[n=3 prob=~[.~0.75 .~0.75 .~1] alias=~[2 2 2]])
    !>((build:alias:sample:rand ~[.~1 .~1 .~2]))
::  +alias +build: the other worklist-draining order (large empties last).
++  test-alias-build-other-order  ^-  tang
  %+  expect-eq
    !>(`alias-table:sample:rand`[n=4 prob=~[.~1 .~0.6666666666666666 .~0.6666666666666666 .~0.6666666666666666] alias=~[0 0 0 0]])
    !>((build:alias:sample:rand ~[.~3 .~1 .~1 .~1]))
++  test-alias-build-empty  ^-  tang
  %-  expect-fail
  |.((build:alias:sample:rand ~))
++  test-alias-build-bad-prob  ^-  tang
  %-  expect-fail
  |.((build:alias:sample:rand ~[.~0 .~0]))
::  +alias +draw: value regression, from the table above.
++  test-alias-draw  ^-  tang
  =/  t  (build:alias:sample:rand ~[.~1 .~1 .~2])
  %+  expect-eq
    !>(`[out=@ud rng:rand]`[2 [%sm64 s=0x3c6e.f372.fe94.f82a]])
    !>((draw:alias:sample:rand t (from-atom:seed:rand %sm64 0)))
...
++  test-categorical  ^-  tang
  ...
  %+  expect-eq
    !>((draw:alias:sample:rand t (from-atom:seed:rand %sm64 0)))
    !>((categorical:rd:dist:rand (from-atom:seed:rand %sm64 0) t))
...
++  test-poisson-knuth  ^-  tang
  %+  expect-eq
    !>(`@ud`2)
    !>(-:(poisson:rd:dist:rand (from-atom:seed:rand %sm64 0) .~5))
++  test-poisson-ptrs  ^-  tang
  %+  expect-eq
    !>(`@ud`14)
    !>(-:(poisson:rd:dist:rand (from-atom:seed:rand %sm64 0) .~20))
++  test-poisson-bad-rate  ^-  tang
  %-  expect-fail
  |.((poisson:rd:dist:rand (from-atom:seed:rand %sm64 0) .~0))
...
++  test-binomial  ^-  tang
  %+  expect-eq
    !>(`@ud`2)
    !>(-:(binomial:rd:dist:rand (from-atom:seed:rand %sm64 0) 20 .~0.3))
++  test-binomial-n-too-large  ^-  tang
  %-  expect-fail
  |.((binomial:rd:dist:rand (from-atom:seed:rand %sm64 0) 1.000.000 .~0.5))
...
::  empty alphas list.
++  test-dirichlet  ^-  tang
  =/  out  `(list @rd)`-:(dirichlet:rd:dist:rand (from-atom:seed:rand %sm64 0) ~[.~1 .~2 .~3])
  =/  mth  ~(. rd:math [%n .~1e-13 .~0])
...
++  test-dirichlet-empty  ^-  tang
  %-  expect-fail
  |.((dirichlet:rd:dist:rand (from-atom:seed:rand %sm64 0) ~))
...
++  test-rs-normal  ^-  tang
  %+  expect-eq
    !>(`@rs`.-0.5933847)
    !>(out:(normal:rs:dist:rand (from-atom:seed:rand %sm64 0)))
++  test-rs-gamma  ^-  tang
  %+  expect-eq
    !>(`@rs`.1.0119848)
    !>(out:(gamma:rs:dist:rand (from-atom:seed:rand %sm64 0) .2))
++  test-rs-expon  ^-  tang
  %+  expect-eq
    !>(`@rs`.1.0752765)
    !>(out:(expon:rs:dist:rand (from-atom:seed:rand %sm64 0) .2))
++  test-rs-poisson-ptrs  ^-  tang
  %+  expect-eq
    !>(`@ud`14)
    !>(out:(poisson:rs:dist:rand (from-atom:seed:rand %sm64 0) .20))
++  test-rs-binomial  ^-  tang
  %+  expect-eq
    !>(`@ud`4)
    !>(out:(binomial:rs:dist:rand (from-atom:seed:rand %sm64 0) 20 .0.3))
++  test-rs-dirichlet  ^-  tang
  =/  out  `(list @rs)`out:(dirichlet:rs:dist:rand (from-atom:seed:rand %sm64 0) ~[.1 .2 .3])
