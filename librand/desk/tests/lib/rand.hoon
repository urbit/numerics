  ::  /tests/lib/rand
::::
::    /lib/rand (the plumbing): ++split-mix, ++philox, ++seed, +fork,
::    ++gen, ++uni (integer: +bits/+below/+between), ++pcg, ++sample
::    (shuffle/permutation/choice/reservoir -- +alias moved to
::    /lib/i754rand, tested there instead).  The SplitMix64 and
...
::    Python re-implementation before being transcribed here.  ++pcg's
::    KAT (seed=42, seq=54, pcg-c's own demo convention) is checked
::    against an independent Python re-implementation of pcg-c's
...
::    earlier spec draft had it backwards).  +from-atom, +fold-wide,
::    +mix, +fork, +bits, and +below have no external reference (they
::    are this library's own design, or Lemire's algorithm applied to
::    this library's own +step), so those are checked by direct
::    computation of the spec'd formula and by determinism/order-
::    sensitivity/path-sensitivity/unbiasedness properties instead.
::
/+  *test,
    rand
|%
