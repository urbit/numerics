...
++  la
  ...
  ++  argmax
    ...
    ^-  @ud
    ?>  (check a)
    +:(find ~[(get-item (max a) (reap (lent shape.meta.a) 0))] (ravel a))
  ::
  ::    +min:  ray -> ray
  ::
  ...
  ++  argmin
    ...
    ^-  @ud
    ?>  (check a)
    +:(find ~[(get-item (min a) (reap (lent shape.meta.a) 0))] (ravel a))
  ::
  ::    +cumsum:  ray -> ray
  ::
...
::
  ...
  ::  - %i754: a - b*san(need(toi(a/b))) in the active door mode; a zero divisor
  ::    or non-finite quotient crashes ((need ~)), matching arvo lib/lagoon.
